(ns insurity.main
  "Kotodama WASM entrypoint for the Insurity clean-room actor (L4) — Clojure port.
  ...
  facts (`insurity.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "insurity")
(def tier "L4")
...
(def entity-specs
  [{:entity "Account"     :plural "accounts"     :id-prefix "insurity_acc"
    :fields [:holderId :type :balance :currency :status]     :required [:type :balance]
    :coerce {:balance :float}                :refs {}}
   {:entity "Loan"        :plural "loans"        :id-prefix "insurity_loa"
...
(defn emit-facts
  "EAVT facts for one record: {\"insurity.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "insurity-compat" :tier tier :entities entities} 200])
...
(defn start! [] :insurity-compat/ready)
